...
void DrawAll(PPLAYER pPlayer)
{
	...
	if (g_sightMode)
	{
		...
		nRight = pPlayer->m_tPos.y + sightRange < MAP_WIDTH_MAX ? pPlayer->m_tPos.y + sightRange : MAP_WIDTH_MAX - 1;
		nBottom = pPlayer->m_tPos.x + sightRange < MAP_WIDTH_MAX ? pPlayer->m_tPos.x + sightRange : MAP_HEIGHT_MAX - 1;
